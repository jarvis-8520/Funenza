...
// --- 3. COLLISION DETECTION ---
    ...
    for (const g of ghosts) {
      const hitDistance = Math.hypot(pacX - g.x, pacY - g.y);
      if (hitDistance < PACMAN_SIZE - 2) { 
        if (frightenedMode && !g.respawning) {
          // PACMAN EATS GHOST
          g.respawning = true;
          
          // 1. Teleport to a random safe corridor path
          const escapeCell = corridorPaths[Math.floor(Math.random() * corridorPaths.length)];
          g.c = escapeCell.c; 
          g.r = escapeCell.r;
          g.targetC = escapeCell.c; 
          g.targetR = escapeCell.r;
          g.x = escapeCell.c * DOT_SIZE + 10; 
          g.y = escapeCell.r * DOT_SIZE + 10;
          
          // 2. Update visual styling for "eaten" state
  ...
  function init() {
    ...
    grid = document.getElementById('zones-grid');
    items = window.zoneGameBoard.cards;;
